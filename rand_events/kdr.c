...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 kdr C:/Users/Viktor/Desktop/Emory/PV_IN/Final_parameters/kdr.mod\n");
...
static int  trates (  double _lv ) {
 ...
 _lqt = pow( q10 , ( ( celsius - 23.0 ) / 10.0 ) ) ;
   minf = - 1.0 / ( 1.0 + exp ( ( _lv + 5.0 + vshift ) / 12.0 ) ) + 1.0 ;
   mtau = tshift + 0.5 + 4.0 * exp ( - 0.5 * pow( ( ( _lv + 25.0 ) / 25.0 ) , 2.0 ) ) ;
   hinf = 1.0 / ( 1.0 + exp ( ( _lv + 30.0 + vshift ) / 10.0 ) ) ;
    return 0; }
 ...
 v=_v;
{
  ek = _ion_ek;
 { error =  states();
 if(error){fprintf(stderr,"at line 49 in file kdr.mod:\n        SOLVE states METHOD cnexp\n"); nrn_complain(_p); abort_run(error);}
 } }}
